...
#                       - with 5 corresponding style factors (Style factors are drivers of return within an asset class):
...
``` 
 ...
 #CUSIP= Committee on Uniform Security Identification Procedures (=Wertpapierkennnummer)
 #TICKER= Abkürzungen an der Börse
...
``` 
## Interpretation:
1) Upper Plot: Mean R-Squared across periods = 0.78 (Output of the Junk) -> problem, R-Squared increases everytime an independant variable is added, which means the more independant variables the higher the R-Squared.
often it is referred to that stocks with >=80% follow the index, while values <70% mean that it does not generally follow an index. e.g. R²=100 & beta<1 means there is a good risk adjusted return possibility. 

2) Mean adjusted R-Squared = 0.54 (Output of the Junk) -> as soon as we take the problem with R-Squared into account and use the adjusted mean, we see that the value drops from 78% to 54% and that there are even negative single values.
R² doesn't take into account that every new predictior increases the R² but never decreases it -> more terms better R² value -> R² adjusted takes this into account. In this example we see that there is less correlation than expected from the R².
...
## Interpretation:  
Multicollinarity reduces the statistical significance of the independent variable. For such a colliniar relationship to other variables should be adjusted in the regression model. There are different standards (cutoffs) when a VIF indicates a high correlation between the variables. I use x<=1 not correlated; 1<x<=5 moderate correlation; x>5 correlated. Therefore, there is no strong multicollinarity in this data set.  
...
## Interpretation:
Market
3 Style factors: Size, P2B & EV2S
8 Market sectors: COSTAP, ENERGY, FINS, HEALTH, INDUST, INFOTK, MATRLS, TELCOM
The red line shows the critical values of the t-statistic. That means everthingy larger (absolut value) than the critical value is significant. Result of collinearity --> not many significant parameters
Gausche- Glockenkurve zeichnen!
...
``` 
## Interpretation: 
As we saw in the t-statistic value (previous graphic), the overall significance is very low a part from the market.
...
The sector factors don't have a volatility because the values which we see are the mean values.
...
``` 
## Interpretation:
Looking at the Factor exposure helps to understand where risks come from. (hier hohe Exposure zu Size, medium zu P2B und negative zu EV2S)
...
#### 2.2) repReturn 
The function repReturn provides you with the following choices of graphical reports, the results of which will also be printed because of the default printing option isPrint = T:
...
``` 
## Interpretation: 
Mean and Volatility of the three stlye factors (size, P2B, EV2S), of the market and of the 8 sector factors.
...
``` 
## Interpretation: 
1. Portfolio Returns of DJ Portfolio
2. Factor Returns (means of the three stlye factors (size, P2B, EV2S), market and of the 8 sector factors)
3. Residual Returns (volatility of the three stlye factors (size, P2B, EV2S), market and of the 8 sector factors)
...
``` 
Ausreißer= outliers /extrem values
...
- expected shortfall (ES) (Der Expected Shortfall gehört daher zu den bedingten Shortfall-Erwartungswerten und wird als Conditional VaR (CVaR), Conditional Tail Expectation oder Expected Tail Loss bezeichnet.)
...
## Interpretation: 
factor percent contribution to risk - (FPCR)
systemetic risk= Size, P2B, EV2S
idiosyncretic risk= Residuals
alpha= 
How many % of SD are determined by size, P2B, EV2S, residuals
...
``` 
## Interpretation:
How many % of expected shortfall are determined by size, P2B, EV2S, residuals
...
``` 
## Interpretation: 
Comparison of SD, ES, VaR
Value at Risk= optimistic view  (VaR= meistens 5%, d.h. 95% of the returns exceeds the VaR)
expected shortfall= realistic view
...
``` 
## Interpretation:
1. Zeile: observed returns
2. Zeile: simulated returns
the simulated returns aren't to bad.
...
``` 
## Interpretation:
exactly the same thing only with standard deviations
...
# First we use fmmcSemiParam with default choice resid.dist = “normal” and with resid.par a matrix with first column the sample mean of the residuals and second column the standard deviation of the residuals 
...
``` 
## Interpretation:
We do exactly the same as before except for adding the interception T (with the market component), so we get the observed industry factors returns. 
...
``` 
## Interpretation: 
If we calculate the industry factors returns together we get 0, due to the relationship of the different sectors.
Relationship Gold/Aktien: Aktien fallen (-10), Gold steigt (+10) --> -10+10=0
...
``` 
## Interpretation
QQ-Plot: Quantiles of two statistical variables are plotted against each other to compare their distributions.
x1-Achse: Standard deviation
x2-Achse: Returns
...
qqnorm(r.mic[[1]],
       main = "MIC Model Equity Returns for First Period", 
       xlab = "NORMAL QQ-PLOT",
       ylab = "RETURNS");qqline(r.mic[[1]], col="red")
